...
void loop() {
  ...
  if (digitalRead(b1) == HIGH) {
    IrSender.sendSamsung(0x7, 0x6C, 0); // A
    digitalWrite(LED, HIGH);
    
  }

  if (digitalRead(b2) == HIGH) {
    IrSender.sendSamsung(0x7, 0x14, 0); // B
    digitalWrite(LED, HIGH);
    
  }

  if (digitalRead(b3) == HIGH) {
    IrSender.sendSamsung(0x7, 0x65, 0); // LEFT
    digitalWrite(LED, HIGH);
    
  }

  if (digitalRead(b4) == HIGH) {
    IrSender.sendSamsung(0x7, 0x60, 0); // UP
    digitalWrite(LED, HIGH);
    
  }

  if (digitalRead(b5) == HIGH) {
    IrSender.sendSamsung(0x7, 0x62, 0); // RIGHT
    digitalWrite(LED, HIGH);
    
  }

  if (digitalRead(b6) == HIGH) {
    IrSender.sendSamsung(0x7, 0x61, 0); // DOWN
    digitalWrite(LED, HIGH);
